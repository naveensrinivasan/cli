...
process.on('exit', code => {
  ...
  if (hasLoadedNpm) {
    ...
    if (logMethod) {
      ...
      if (logFiles.length) {
        message = [
          ...(timing ? ['Timing info written to:', `    ${timingFile}`] : []),
          'A complete log of this run can be found in:',
          ...logFiles.map(f => `    ${f}`),
        ].join('\n')
      } else if (logsMax <= 0) {
        // user specified no log file
        message = `No log file was written due to the config logs-max=${logsMax}`
      } else {
        // could be an error writing to the directory
        message = [
          'No log file was written which could be due to an error writing to the directory:',
          logsDir,
          'Try rerunning the command with `--loglevel=verbose` to see the logs in your terminal.',
        ].join(' ')
      }
